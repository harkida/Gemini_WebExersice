...
[채점 기준: 4단계 다면 평가 기준표]
- 10.0점에서 시작하여, 아래 기준에 따라 오류의 심각도에 맞춰 점수를 차감합니다. 각 오류 유형에 대한 구체적인 예시를 반드시 숙지하고 적용해야 합니다.

- [1단계: 치명적 오류 (Critical Errors)] (-4.0점 ~ -6.0점 감점)
  - 정의: 문장의 핵심 의미가 완전히 왜곡되거나 반대로 해석된 경우.
  - 예시:
    - 핵심 동사 오역 (예: '먹다'를 '자다'로 번역)
    - 핵심 명사 오역 (예: '학교'를 '병원'으로 번역)
    - 긍정/부정 오역 (예: '했다'를 '안 했다'로 번역)
    - 핵심 시간/장소 오역 (예: '어제'를 '내일'로, '여기'를 '저기'로 번역)

- [2단계: 주요 오류 (Major Errors)] (-2.0점 ~ -3.0점 감점)
  - 정의: 문장의 핵심 의미는 파악했으나, 의미를 구성하는 중요 요소가 누락되거나 잘못 해석된 경우.
  - 예시:
    - 문장 유형 혼동 (예: 평서문을 의문문으로, 또는 그 반대로 번역)
    - 핵심 수식어 누락 또는 오역: 문장의 뉘앙스가 아닌, 의미 자체에 큰 영향을 주는 부사/형용사. (예: '자연스럽게 번역하세요'에서 '자연스럽게'를 누락, '빨리 오세요'에서 '빨리'를 누락)
    - 주요 접속사 오역 (예: '그래서'를 '그러나'로 번역)
    - 일부 디테일 누락 (예: '가방 안에 책과 연필이 있다'를 '가방 안에 책이 있다'로 번역)

- [3단계: 경미한 오류 (Minor Errors)] (-0.5점 ~ -1.5점 감점)
  - 정의: 문장의 핵심 의미와 주요 요소는 모두 정확하지만, 덜 중요한 세부 정보나 수식어가 누락된 경우.
  - 예시:
    - 덜 중요한 수식어 누락 (예: '아주 맛있는'을 그냥 '맛있는'으로 번역)

- [4단계: 뉘앙스 분석 (Nuance Analysis)] (감점 없음 또는 최대 -0.5점)
  - 정의: 번역 자체는 틀리지 않았으나, 한국어 원문이 가진 미묘한 뉘앙스를 완벽하게 살리지 못한 경우. 이 단계는 감점보다 '교사용 참고 자료' 생성에 중점을 둡니다.
  - 지시: 분석 결과를 반드시 `[교사용 참고]` 태그를 붙여 보고해야 합니다.
  - 예시:
    - 의도/의지의 차이 (예: '-(으)ㄹ게요'와 '-(으)ㄹ 것이다'의 뉘앙스 차이)
    - 추측의 강도 차이 (예: '-것 같다'와 '-것이다'의 뉘앙스 차이)
    - 존댓말 단계의 미세한 차이 (이탈리아어 번역상 큰 차이가 없다면 감점하지 않음)
