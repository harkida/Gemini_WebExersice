...
**원칙:** 어휘 선택이 곧 의미 전달이다.
이 항목은 오로지 "학생이 올바른 어휘를 선택했는가"만 평가한다.
활용/형태 오류(예: "졸업으려면")는 여기서 평가하지 않는다. 문법 정확성에서 별도 평가.

---
**평가 방법:** 각 목표 어휘의 "선택" 자체만 Gate A/B/C 판정 → 합산 → Gate C 비율 감점
**어휘당 기본 배점** = 5.5 ÷ 목표 어휘 수
---

**Gate A — 올바른 선택 (배점 × 100%)**
목표 어휘 또는 자연스러운 유의어를 선택함. 활용/형태 오류는 무관.
  - "성적" → "성적" (정확)
  - "성적" → "점수" (유의어, 의미 전달됨)
  - "졸업하다" → "졸업으" (어휘 선택은 맞음. 활용 오류는 문법에서 평가)
  - teacher_criterion에 유의어 범위가 명시되어 있으면 그것을 우선 따름

**Gate B — 미사용 (배점 × 0%)**
해당 목표 어휘를 아예 사용하지 않음. 관련 정보가 문장에서 빠짐.
**빠진 것이지, 틀린 것은 아니다.**
  - 예상 정답에 "성적"이 있지만 학생이 아예 언급하지 않음

**Gate C — 의미 왜곡 오사용 (배점 × 0% + 비율 감점)**
목표 어휘 대신 **완전히 다른 의미의 어휘**를 사용하여 문장의 의미가 달라짐.
한국어 원어민이 들었을 때 학생의 의도를 이해할 수 없거나,
전혀 다른 상황으로 해석하게 되는 경우.
**"안 쓴 것(Gate B)"보다 "잘못 쓴 것(Gate C)"이 항상 더 심각하다.**
빠진 정보는 추론할 여지라도 있지만, 틀린 정보는 오해를 만들기 때문이다.

  Gate C 비율 감점 (기본 점수에서 추가 차감):
  - Gate C 비율 = Gate C 어휘 수 ÷ 전체 목표 어휘 수
  - 추가 감점 = 기본 점수 × Gate C 비율
  - 어휘 적합성 최종 점수는 최저 0.0점 (음수 불가)

---

**Gate C 판정 예시 (해당):**

  [C-1: L1 간섭 — false friend]
  목표 어휘: "성적"
  학생 사용: "투표"
  → 이탈리아어 'voto'가 '성적'과 '투표' 두 의미를 갖지만,
    한국어에서 "투표"는 선거 행위. 완전히 다른 의미. → Gate C

  [C-2: 핵심 동사 방향 전환]
  목표 어휘: "가르치다"
  학생 사용: "배우다"
  → 가르침과 배움은 행위 주체와 방향이 반대. → Gate C

  [C-3: 핵심 명사 대상 전환]
  목표 어휘: "머리"
  학생 사용: "배"
  → 아픈 부위가 완전히 다름. → Gate C
...
**Gate C가 아닌 경우 (주의!):**
  ...
  [반례 C-R2: 활용 오류]
  목표 어휘: "졸업하다" / 학생 사용: "졸업으"
  → 어휘 선택은 맞음. Gate A. (활용 오류는 문법에서 평가)

  [반례 C-R3: 표현 방식 차이]
  예상: "성적이 좋아야 해요" / 학생: "성적이 있어야 해요"
  → "좋다/있다"는 뉘앙스 차이이지 의미 왜곡이 아님. Gate A.

---
**⚠️ 특별 규칙: 완전 불일치**
PHASE 1 인식 텍스트가 상황/예상 정답과 **완전히 무관한 내용**이면:
  - 어휘 적합성: **0.0 / 5.5점** (Gate 판정 생략)
  - evaluation에 명시: "학생이 상황과 완전히 무관한 내용을 말함"
---

**채점 계산 과정 (evaluation에 반드시 표시할 것):**

  Step 1. 목표 어휘 수 확인 → 어휘당 배점 계산 (5.5 ÷ 어휘 수)
  Step 2. 각 어휘 Gate A/B/C 판정 (근거 명시)
  Step 3. 기본 점수 = Σ(어휘당 배점 × Gate 비율)
  Step 4. Gate C 비율 감점 = 기본 점수 × (Gate C 수 ÷ 전체 어휘 수)
  Step 5. 최종 어휘 점수 = 기본 점수 - Gate C 비율 감점 (최저 0.0)
...
**실제 계산 예시:**
  ...
  [예시 1: Gate C 포함]
  목표 어휘: 졸업하다, 성적, -(으)려면 (3개)
  어휘당 배점: 5.5 ÷ 3 ≈ 1.83

  · 졸업하다 → Gate A (어휘 선택 정확) → 1.83 × 1.0 = 1.83
  · -(으)려면 → Gate A (어휘 선택 정확) → 1.83 × 1.0 = 1.83
  · 성적 → Gate C ("투표" 사용, 의미 왜곡) → 1.83 × 0.0 = 0.00

  기본 점수: 1.83 + 1.83 + 0.00 = 3.66
  Gate C 비율 감점: 3.66 × (1/3) = 1.22
  최종 어휘 점수: 3.66 - 1.22 = 2.44 / 5.5점

  [예시 2: Gate B만 있는 경우 (의미 왜곡 없음)]
  목표 어휘: 졸업하다, 성적, -(으)려면 (3개)
  ...
  → Gate B(미사용) = 3.66점, Gate C(의미 왜곡) = 2.44점.
    같은 어휘를 못 쓴 상황이지만, 차이는 1.22점.
    이것이 "안 쓴 것 vs 잘못 쓴 것"의 차이다.
...
1.5점 만점에서 시작하여 오류 유형별 가중치에 따라 감점한다.
각 오류는 심각도에 따라 해당 가중치 범위 내에서 감점한다.

**오류 유형별 가중치:**

1. **의미 혼동 (50% = 최대 -0.75점)**
   발음 오류로 인해 단어의 의미가 완전히 달라지는 경우.
   - 예: "멋있다" → "맛있다" (완전히 다른 의미)
   - 예: "닫다" → "다다" (의미 불명)
   - 감점 범위: -0.4 ~ -0.75점 (오류 심각도에 따라)

2. **중간 오류 (30% = 최대 -0.45점)**
   의미는 통하지만 발음이 부자연스러운 경우.
   - 경음화 오류: "사랑해요" → "싸랑해요"
   - 자음 혼동: "자다" → "차다"
   - 감점 범위: -0.2 ~ -0.45점 (오류 심각도에 따라)

3. **극심한 오류 (20% = 최대 -0.30점)**
   거의 알아들을 수 없는 수준.
   - 예: "안녕하세요" → "아나세요"
   - 감점 범위: -0.15 ~ -0.30점 (오류 심각도에 따라)

4. **사소한 오류 (감점 없음, 피드백만 제공)**
   - ㅈ/ㅊ 미세 차이
   - 억양 부자연스러움
   - 가중치 대상 아님. evaluation과 feedback에 기록만 함.

**⚠️ 특별 규칙:**
- PHASE 1에서 인식한 텍스트가 기준임
- "학생이 X라고 발음했으나, Y여야 함"으로 기록
- 절대 `recognized_text`를 수정하지 말 것!
- 상황과 완전히 무관한 문장이면 발음 평가만 독립적으로 수행
...
{{
  ...
  "vocabulary_usage": {{
    "목표어휘1": {{
      "gate": "A 또는 B 또는 C",
      "student_used": "학생이 실제 사용한 어휘 (Gate B이면 null)",
      "note": "판정 근거"
    }},
    "목표어휘2": {{
      "gate": "A",
  ...
  "vocabulary_score_detail": {{
    "per_vocab_point": 1.83,
    "base_score": 3.66,
    "gate_c_count": 1,
    "gate_c_ratio_deduction": 1.22,
    "final_vocabulary_score": 2.44
  }},
...
- [ ] 각 목표 어휘에 대해 Gate A/B/C 판정을 수행했는가?
- [ ] Gate C 판정 시 의미 왜곡 근거를 명시했는가?
- [ ] Gate C 비율 감점을 정확히 계산했는가?
- [ ] vocabulary_score_detail에 계산 과정이 기록되었는가?
