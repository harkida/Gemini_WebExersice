...
1.  만점(10.0)에서 시작한다.
2.  아래 기준에 따라 오류를 발견할 때마다 점수를 차감한다.
    -   **완전한 오역 또는 의미 왜곡:** 원문의 핵심 의미를 완전히 잘못 이해하여 정반대의 의미나 전혀 다른 의미로 번역한 경우. (감점: -6.0 ~ -8.0점)
    -   **핵심 정보 누락/오류:** 문장의 주어, 목적어, 동사 등 핵심적인 구성 요소나 정보를 빠뜨리거나 틀리게 번역한 경우. (감점: -3.0 ~ -5.0점)
    -   **사소한 의미 불일치:** 전체적인 의미는 맞지만, 특정 단어나 표현의 뉘앙스를 잘못 이해하여 약간의 의미 차이가 발생한 경우. (감점: -1.0 ~ -2.0점)

3.  **뉘앙스 및 격식 (Nuance & Formality):**
